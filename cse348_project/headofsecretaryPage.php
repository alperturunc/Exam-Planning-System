...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if ($_POST['form_type'] == 'create_course' && isset($_POST['coursecode']) && isset($_POST['semester'])) {
        ...
        $sql_assistants_in_faculty = "SELECT employee.employee_id,employee.employee_name,employee.employee_score
        FROM employee
        JOIN department
        ON employee.department_id = department.department_id
        WHERE employee.employee_type = 'Assistant' AND department.faculty_id = '$current_faculty'
        ORDER BY employee.employee_score ASC";  // Query to add selecting all the assistants in the faculty
        $result_assistants_in_faculty = mysqli_query($conn, $sql_assistants_in_faculty);

        $counter = 0;
        $observers = [];

        while ($assistant = mysqli_fetch_array($result_assistants_in_faculty)) {
            if ($counter == $exam_assistant_num) {
                ...
                if (mysqli_num_rows($result_lectures) > 0 || mysqli_num_rows($result_exams) > 0 ) {
        ...
        <div class="examform-container">
            ...
            <div class="assistant_list">
                ...
                <table>
                    ...
                    <tbody>
                        <?php 
                        $sql_scores = "SELECT employee_name, employee_score 
                        FROM employee 
                        JOIN department
                        ON employee.department_id = department.department_id
                        WHERE employee_type = 'Assistant' AND department.faculty_id = '$current_faculty'
                        ORDER BY employee.employee_score ASC"; //Query to add the assistants scores in the faculty
